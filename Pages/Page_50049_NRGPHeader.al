page 50049 "NRGP Header"
{
    ...
    begin
        ...
        if PostedNRdcHdr.FindFirst() then begin
            PostedNRdcHdr.Init();
            IF Rec."Global Dimension 1 Code" = 'DOM' then
                PostedNRdcHdr."No." := NoseriesMgmt.GetNextNo('NRDC-DOM', WorkDate(), true);

            IF Rec."Global Dimension 1 Code" = 'EOU' then
                PostedNRdcHdr."No." := NoseriesMgmt.GetNextNo('NRDC -EOU', WorkDate(), true);

            IF (Rec."Global Dimension 1 Code" = 'DOM') then begin
                IF ((REC."Consignee Type" = Rec."Consignee Type"::Customer) AND (REC."Reference Type" = Rec."Reference Type"::"Sales Shipment")) THEN
                    PostedNRdcHdr."No." := NoseriesMgmt.GetNextNo('NRDC-DOM-SDC', WorkDate(), true)
            END;
            IF (Rec."Global Dimension 1 Code" = 'EOU') then begin
                IF ((REC."Consignee Type" = Rec."Consignee Type"::Customer) AND (REC."Reference Type" = Rec."Reference Type"::"Sales Shipment")) THEN
                    PostedNRdcHdr."No." := NoseriesMgmt.GetNextNo('NRDC-EOU-SDC', WorkDate(), true)
            END;

            IF (Rec."Global Dimension 1 Code" = 'DOM') then begin
                IF ((REC."Consignee Type" = Rec."Consignee Type"::Vendor) AND (REC."Reference Type" = Rec."Reference Type"::"Transfer Shipment")) THEN
