            ...
            repeater(Control1)
            {
                ...
                field("To Item No."; Rec."To Item No.")
                {
                    ...
                    begin
                        ...
                        if (CopyStr(Rec."To Item No.", 1, 1) <> 'E') and (CopyStr(Rec."To Item No.", 1, 1) <> 'D') then
                            Error('You cannot Select this item');
                        IF ItemRec.GET(Rec."To Item No.") THEN BEGIN
                            Rec."To Item Description" := ItemRec.Description;
                            //"Specification ID" := ItemRec."Spec ID";
                            DefaultDimension.Reset();
                            DefaultDimension.SetRange("Table ID", 27);
                            DefaultDimension.SetRange("No.", Rec."To Item No.");
                            DefaultDimension.SetRange("Dimension Code", GLSetup."Shortcut Dimension 1 Code");
                            if DefaultDimension.FindFirst() then
                                Rec."New Shortcut Dimension 1 Code" := DefaultDimension."Dimension Value Code";
                        END;
                        ...
                        PurchInvLine.SETRANGE("No.", Rec."From Item No.");
                        IF PurchInvLine.FINDLAST THEN
                            Rec."Unit Cost" := PurchInvLine."Unit Cost (LCY)";
                    end;
                }
