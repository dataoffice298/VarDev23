pageextension 50065 InspectionReceiptExt extends "Inspection Receipt B2B"
{
    layout
    {
        ...
        addlast(Receipt)
        {
            ...
            field("No. Of Samples Taken"; Rec."No. Of Samples Taken")
            {
                ApplicationArea = all;
            }
        }
    ...
    actions
    {
        modify("P&ost")
        {
            ...
            begin
                if Rec."Vendor Test Certificate_B2B" or Rec."Warranty Certificate_B2B" then
                    Rec.TestField("QC Certificate(s) Status", Rec."QC Certificate(s) Status"::Available);
            end;
        }
    }
    trigger OnAfterGetCurrRecord()
    begin
        if (not Rec."Vendor Test Certificate_B2B") and (not Rec."Warranty Certificate_B2B") then begin
            Rec."QC Certificate(s) Status" := Rec."QC Certificate(s) Status"::"Not Required";
            //Modify();
        end else begin
            Rec."QC Certificate(s) Status" := Rec."QC Certificate(s) Status"::" ";
            // Modify();
        end;
        if Rec."QC Certificate(s) Status" = Rec."QC Certificate(s) Status"::"Not Required" then
            BoolGvar := false
        else
            BoolGvar := true;
    end;
